...
public final class MonitorSample

{
	// ---( internal utility methods )---

	final static MonitorSample _instance = new MonitorSample();

	static MonitorSample _newInstance() { return new MonitorSample(); }

	static MonitorSample _cast(Object o) { return (MonitorSample)o; }
	...
	public static final void monitorQueue (IData pipeline)
        throws ServiceException
	{
		// --- <<IS-START(monitorQueue)>> ---
		// @sigtype java 3.5
		...
		String queue = IDataUtil.getString(pipelineCursor, "queueName");
		...
		try {
		    ...
		    nQueueNode nq = (nQueueNode) realm.findNode(queue);
		    System.out.println(nq.getDepth());
		    
		    
		    IDataUtil.put(pipeline.getCursor(), "qLength", nq.getDepth());
		    IDataUtil.put(pipeline.getCursor(), "published", nq.getTotalPublished());
		    IDataUtil.put(pipeline.getCursor(), "consumed", nq.getTotalConsumed());
	...
	public static final void monitorSample (IData pipeline)
        throws ServiceException
	{
		// --- <<IS-START(monitorSample)>> ---
		// @sigtype java 3.5
		// [i] field:0:required realmURL
		...
		String keyStorePath= IDataUtil.getString(pipelineCursor, "keyStorePath");
		String keyStorePass=IDataUtil.getString(pipelineCursor, "keyStorePass");
		String keyStoreAliaIDataUtil.put(pipeline.getCursor(), "qLength", nq.getDepth());IDataUtil.put(pipeline.getCursor(), "qLength", nq.getDepth()); IDataUtil.put(pipeline.getCursor(), "qLength", nq.getDepth()); IDataUtil.put(pipeline.getCursor(), "qLength", nq.getDepth()); IDataUtil.put(pipeline.getCursor(), "qLength", nq.getDepth());IDataUtil.put(pipeline.getCursor(), "qLength", nq.getDepth());s =IDataUtil.getString(pipelineCursor, "alias");
		String trustStorePath= IDataUtil.getString(pipelineCursor, "trustStorePath");
		String trustStorePass =IDataUtil.getString(pipelineCursor, "trustStorePass");
		if (!StringUtils.isEmpty(keyStorePath)) {
			System.getProperties().put("com.softwareag.um.client.ssl.keystore_path", keyStorePath);
		...
		try {
		    ...
		    for (nNode n: channels) {
		        ...
		        try {
		            Thread.sleep(2000);// Wait for the API to gather all the data from the channel.
